...
- 交付物：yd 流域 SHUD 产物展示服务（viewer）+ node-22 循环预报环（compute），服务地方客户；最终部署在**客户服务器，交付后我们无法登录运维**——一切以离线自足为设计前提。
- 技术栈：Python/FastAPI（uv）· Vite+React+TypeScript+MapLibre GL+ECharts（pnpm）· **无数据库、无写路径**（viewer 侧）· Docker 多阶段构建 + 离线 bundle 交付
- 权威文档（实现与文档冲突时以文档为准；契约/设计变更**先改 docs 再动码**）：
  - [docs/design.md](docs/design.md) — viewer 方案、API、里程碑 M1–M5
  - [docs/compute-loop-design.md](docs/compute-loop-design.md) — node-22 循环预报环（双源 IFS+GFS、12h 节律、两段跑 warm start）
  - [docs/products-contract.md](docs/products-contract.md) — 产物目录契约 v1（唯一生产者=循环环，唯一消费者=viewer）
