...
- 数据层（取数期）：先过结构层，再整读文件；第 0 列逐值校验为由本节常量推导的 `i*60`（`i=0..167`），包括拒绝非有限分钟值。生产读取与测试写出独立推导、不共用同一算术表达式；独立 golden 分钟轴须能检出 validator 偏移 +60，以及 writer/validator 同时偏移 +60 的共同错误；数学等价表达式替换不要求令行为测试失败。失败抛出带路径与原因的 `DatError`，不返回部分结果；API 回落/省略规则见 [design.md](design.md) §6.1。
