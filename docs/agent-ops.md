...
- 2026-09-24 cron 日志格式更新（授权，15:39Z）：checkout `a1073e3` → `ffb72a0`（#347 PR #349，compute-loop §6「`run` stderr 行格式」），按 §6 porcelain 干净后 `git pull --ff-only`，区间内 producer 仅 `cli.py` 与新测试、依赖不变，更新时无在跑 tick/作业；16:17Z 首个 tick 写出两行 `2026-09-24T16:17:02Z 等待：ifs/gfs …raw 未齐…`，旧的无前缀「错误：」行不再出现。退出码与 cron 行不变。`receipts/producer-log-format-update-20260924.txt`；回滚为 checkout 回 `a1073e3`。
- 2026-09-25 run 失败路径保留外层 note（授权，05:25Z）：checkout `ffb72a0` → `217a2ca`（#350 PR #365：`_StatesGuardFailed`/`RunSourcesError`/run 下 `OSError` 的 handler 输出 `__notes__`，锁 identity 漂移证据进 `cron.log`），按 §6 porcelain 干净、无在跑 tick/作业后 `git pull --ff-only`，区间内 producer 仅 `cli.py` 与 `test_cli_run_log.py`、依赖不变；更新前 05:17Z tick 为两行「等待」（2026092500 raw 未齐），退出码与 cron 行不变。`receipts/producer-run-notes-update-20260925.txt`；回滚为 checkout 回 `ffb72a0`。
...
- 2026-09-22 `nwm.ac.cn/yd/` 切主线（用户手工 sudo，07:55Z）：`nwm.ac.cn.conf` `/yd/` upstream 8081→8082（备份 `.bak.20260923T075521`，`nginx -t` 后 reload）；公网 `nwm.ac.cn/yd/` 与 `test.nwm.ac.cn/yd/` 均由主线 `yd-web` 提供；副本实例（`:8081`、`yd-*` user units、`yd-db`）仍在运行但不再被 `/yd/` 引用，停用另行裁决。`receipts/m5-nginx-nwm-ac-cn-switch-20260922.txt`。
- 2026-09-23 天地图同源反代重部署（授权，00:45Z）：`yd-viewer:1fdb7fd`（PR #332 `GET /api/basemap/tianditu/{layer}/{z}/{x}/{y}`，本机 linux/amd64 构建，合成数据 + 假 key 代理模式 smoke 全绿）`docker load`；`.env` 追加 `YD_TIANDITU_KEY`（机器内从 NWM `basemap.py` 默认值拷贝，不回显）与 `YD_BASEMAP_CACHE_DIR=/cache`，`compose.yml` 切 tag 并加命名卷 `yd-basemap-cache:/cache`（备份 `.env.bak-20260923T004459Z`、`compose.yml.bak-0ec9fbc-20260923T004459Z`）；容器重建后 `basemaps.json` 六条 URL 为相对 `api/basemap/tianditu/<layer>/{z}/{x}/{y}`；回环 `vec/1/0/0` 首次 `X-Tile-Cache: miss` 再次 `hit`（`image/png`），非法 layer 422、越界 z/x 404，公网 `nwm.ac.cn/yd/api/basemap/tianditu/vec/1/0/0` 200；NWM `:8080`、`nwm.ac.cn/`、NWM 自身 `/api/v1/basemap/tianditu` 前后 200；浏览器 receipt（`shots/06-vector-proxy.png`）矢量底图渲染正常，70 片瓦片全部经反代 200、直连 tianditu 0——先前登记的矢量层限流偏差解除。receipt 经 `tk=` 掩码，key 字面 0 次；旧镜像 0ec9fbc/e23b357 保留供回滚。`receipts/m5-redeploy-1fdb7fd-proxy-20260923.txt`，汇总 md 追记同日。
- 2026-09-24 首页外观对齐 NWM 重部署（授权，14:32Z）：`yd-viewer:bdc837d`（#338 PR #341 页头横栏/图层卡片/底图图标/径流量图例/指南针与版权，#339 PR #343 曲线窗 `M11RiverForecastPanel` 外观与滚轮缩放；design §7 PR #337），本机 linux/amd64 构建，代理模式合成数据 smoke 全绿；仅 `compose.yml` 切 tag（备份 `compose.yml.bak-1fdb7fd-20260924T143252Z`），`.env` 未动，缓存卷保留（184 片）；回环与公网 health 200、标题不变，品牌图 `logo-*.png`/`sponsors-*.png` 在构建物内；NWM `:8080`、`nwm.ac.cn/` 前后 200；浏览器 receipt（`shots/live-bdc837d.png`、`live-bdc837d-click.png`）横栏、卡片起报时间、「© 天地图 | MapLibre」、指南针、曲线窗副标题「河段 Q_DOWN 流量预报 · GFS+IFS」均正常，90 片瓦片经反代 200、直连 tianditu 0、无页面错误；receipt `tk=` 命中 0；旧镜像 1fdb7fd/0ec9fbc/e23b357 保留供回滚。`receipts/m5-redeploy-bdc837d-nwm-look-20260924.txt`。
- 2026-09-24 `.env` 清理（用户要求，14:52Z）：删除六条遗留 `YD_BASEMAP_*_URL`（内嵌 `tk=`，反代模式下已被忽略，§9.2），`.env` 只剩 `YD_VIEWER_INPUT_DIR`/`YD_VIEWER_OUTPUT_DIR`/`YD_VIEWER_PORT`/`YD_TIANDITU_KEY`/`YD_BASEMAP_CACHE_DIR`（600）；`compose up -d` 重建 `yd-web` 后容器环境 `tk=` 由 6 条降为 0，`basemaps.json` 前后字节一致，瓦片 hit、回环/公网 health 200、NWM 前后 200。备份 `.env.bak-20260924T145225Z` 仍含旧行，与更早的 `.env.bak-20260923T004459Z` 一样在确认不再回滚后可删。`receipts/m5-env-cleanup-20260924.txt`。
- 2026-09-25 瓦片缓存与 NWM 共用（授权，04:02Z）：`yd-viewer:39a39c6`（#354 PR #356：命中刷新 mtime、entrypoint `umask 002`；docs #353/#355）本机 linux/amd64 构建（tar.gz sha256 `7f97e518…`）`docker load`；nwm 给 `/home/nwm/.cache/nhms/mvt/basemap/tianditu` 1124 个目录加 setgid（前 1124 无、后 0 无）；`compose.yml` 切 tag、`/cache` 改 bind 该目录、加 `group_add: ["1005"]`、去命名卷声明（备份 `compose.yml.bak-bdc837d-20260925T040215Z`），`.env` 未动；容器 `groups=…,1005`、PID 1 `Umask: 0002`；NWM 写的 `vec/4/8/7` 经 yd 首次请求即 `hit`，mtime 1790213732→1790308939（非属主经组写刷新）；yd 未命中 `vec/15/25775/12794` 新建目录 `2775 :nwm`、文件 `664 :nwm`，二次 `hit`，NWM `/api/v1/basemap/tianditu/…` 同一瓦片 `hit`；无 `tmp-*` 残留、日志无写入失败；回环与两个公网 `/yd/api/health` 200，NWM `/health`、`nwm.ac.cn/` 200（脚本原探 `/api/v1/health` 非 NWM 路由，前后均 404，已追记更正）；浏览器 receipt 三种底图瓦片全部经反代 200、直连 tianditu 0、无页面错误。旧命名卷 `yd-basemap-cache`（3.3M）与镜像 bdc837d 保留供回滚，删除另行授权。`receipts/m5-shared-cache-39a39c6-20260925.txt`。
