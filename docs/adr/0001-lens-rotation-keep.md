...
- 后期复议：[0001-lens-rotation-keep-history-02.md](0001-lens-rotation-keep-history-02.md)（第 20 次起）
...
`docs/review-loop-log.jsonl` 的命中数组键名固定为 `catches`。
早期复议里的归因数字与 `findings` 发现，按当时记录口径保留，不是按当前日志重算的结果。
...
状态仍为**默认 keep、待人工确认**。
2026-09-15（#80）：issue #5 / PR #40 条目的键已改为 catches；历史数字按发现时口径保留。
...
第 20 次及之后的复议见 [0001-lens-rotation-keep-history-02.md](0001-lens-rotation-keep-history-02.md)。
