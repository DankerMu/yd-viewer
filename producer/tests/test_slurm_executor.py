"""`yd_producer.slurm` 契约测试的下半：executor 组装与协议一致性（原 E 段）。

与 `test_slurm.py` 同一套纪律：argv 逐元素精确比对，时间断言用注入 `StepClock` 的确
定性精确值，失败路径以 `pytest.raises(ExecutorError)` 表达并机检 `exc.job_id`。共享
固定输入与假 runner 见 `slurm_fixtures.py`；本文件为满足 1000 行文件上限从
`test_slurm.py` 拆出。
"""
