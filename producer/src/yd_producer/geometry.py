...
* **路径绑定**：`read_shapefile` 打开的每个文件都属于调用方点名的那一组 shapefile。
  ...
  而不是 `yd.shx`）；调用方给的 `.shp` 自身只做校验，绝不二次推导。
...
def read_shapefile(shp_path: str | Path) -> tuple[CRS, list[Feature]]:
    ...
    `shp_path` 必须以 `.shp` 结尾（大小写不敏感），否则抛 `GeometryError` 并点名
    该路径；`.shx`/`.dbf`/`.prj` 只替换该最后一个后缀得到，调用方给的 `.shp`
    自身只做存在性校验，不二次推导。
    """
    ...
    # `.shx` 单独先验一遍（materialize offsets），使索引损坏/截断的报错点名 `.shx`
    # 而不是完好的 `.shp`——合并打开时 pyshp 只会抛出一个笼统异常。
    try:
        with shapefile.Reader(shp=None, shx=str(shx_file)) as reader:
            _ = reader.shx_reader.offsets  # 触发索引记录全量解析
    except Exception as exc:
