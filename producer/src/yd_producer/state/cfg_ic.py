...
def _header_minute_index(header_tokens: Sequence[str]) -> int | None:
    ...
    """
    # NWM@8ae9b8f2 packages/common/state_qc.py:609（逐字移植；pin 侧名为
    # `cfg_ic_header_minute_index`，本仓保持私有——它的公开消费者是重戳，属任务 4.3）
    numeric_indices = [
        index
...
def header_minute_time(header_tokens: Sequence[str]) -> float | None:
    """Return the SHUD IC header minute-time value, or None.

    Uses :func:`_header_minute_index` so the minute-time is read from the
    LAST numeric token regardless of whether a lake count is present.
    """
    # NWM@8ae9b8f2 packages/common/state_qc.py:629（逐字移植；pin 侧名为
    # `cfg_ic_header_minute_time`）。tracker 的轮询判据复用本函数，不另移植一份。
    index = _header_minute_index(header_tokens)
    if index is None:
        return None
