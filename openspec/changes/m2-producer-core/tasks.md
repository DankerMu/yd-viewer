...
### Issue #48 fixture（任务 1.9）

Fixture level: expanded
Upstream suggested level: absent（legacy review-DEFER issue）
Repair intensity / effective tier: high（本 PR 不改生产行为，但它重建三入口共享 config/local reader 的完整 provenance oracle；假绿会让 validate-then-discard 回归穿过全部消费者）
Project profile: yd-viewer
Minimal mergeable slice: 只修改 `producer/tests/test_config.py` 与 task 1.9 / 本 fixture；`config.py`、生产 TOML 与其它测试零 diff

Change surface:
- `producer/tests/test_config.py`：把现有 `test_load_config_returns_all_fields` / `test_load_local_returns_all_site_fields` 从单值 fixture 改为至少两套完整 A/B pair 的参数化 round-trip，并增加 A/B 每个当前输出叶都不同的自证
- 本任务 1.9、共享 #2 fixture 的两条齐备证据与本独立 fixture
- `producer/src/yd_producer/config.py`、`producer/config.toml`、`local.toml`、`producer/tests/cli_fixtures.py` 与其它产品/测试模块均不改

Governing invariant:
- 完整装载 oracle 必须区分“返回文件中本次写入的值”和“照常校验后丢弃结果、返回唯一 fixture 常量”：对每个 Config/LocalConfig 输出叶，至少有两份完整合法输入提供不同值，并由不依赖输入 dict/loader 的 expected 对象分别对拍。
...
- Legacy compatibility / examples: selected - 大量现有测试继续依赖 A；B 只服务完整 provenance oracle
- Error handling / rollback / partial outputs: selected - 既有缺项/类型/domain fail-closed oracle不得削弱，完整合法 pair 都必须成功
- Release / packaging / dependency compatibility: selected - 不改依赖/lock/生产代码
- Documentation / migration notes: selected - 只更新 OpenSpec evidence/fixture；无 userspace 或数据迁移
- Geospatial / CRS: not selected - 无几何
- Time series / forcing / temporal boundaries: selected - A/B 的 cycle/lead/forecast/checkpoint 均不同且合法，但不新增域
- 状态链 / warm-start: not selected - 不读写状态
- NWM 快照溯源 / DB-free 隔离: not selected - NWM module/grid/path 只作合成第二值，不声明现场事实

Invariant Matrix:
- Source of truth: 两份独立完整 TOML 输入 A/B + 两份独立完整 expected dataclass 树；二者不得互相生成
- Producer: `_render_toml` / `_write_toml` 只把选中的完整输入写到 `tmp_path`，不参与 expected
- Validator/read surface: public `load_config` 与 `load_local`
- Downstream consumers: 无改动；完整 producer suite 证明已有 A fixture 消费者与 loader userspace 保持
...
Boundary-surface checklist:
- Shared helper roots: 测试侧 `_render_toml` / `_write_toml`；不得改 production builder
- Public entrypoints/read surfaces: `load_config`、`load_local`
- Write/delete/overwrite and staging/publish/rollback: 仅 pytest `tmp_path`，无产品写面
- Producer/consumer evidence boundaries: TOML input bytes → loader object → independent expected dataclass
- Stale-state/idempotency: none
- Unchanged downstream consumers: producer full suite；`cli_fixtures.py` 与 NWM 专项测试零 diff
...
- 独立失败退出码 provider MUST 接受一个 terminal `JobRecord`，且只接受 `FAILED`/`TIMEOUT`；它恰调用一次 runner，argv 逐元素为 `("sacct", "-j", record.job_id, "-X", "-n", "-P", "--format=ExitCode")`。输出中去除纯空白行后必须恰有一行、恰有一个非空字段，返回该字段的 `strip()` 结果（例如 `"42:7"`）；空、多行、多字段或 runner 异常统一抛 `ExecutorError(job_id=record.job_id)`。provider MUST 使用与 Slurm executor 相同的 runner 与环境/超时政策，但不得修改或复用轮询记录；每次失败收尾只查询一次，不重试。

  #47 allocation-only 更正（PR #197 R197-SI-1）：[Slurm sacct 官方合同](https://slurm.schedmd.com/sacct.html#OPT_allocations)默认同时返回 job 和 job-step 记录，`--format=ExitCode` 只选列、不筛行；`-X` 与既有普通轮询一致，是只取 allocation 的必要选择器。此处更正旧 argv 的遗漏，不改变独立单次 provider、ExitCode 载体、七字段 `JobRecord` 或 M4 现场复核边界；不能通过放宽多行 parser 绕过问题。

  判别证据：同一已失败作业的 allocation=`42:7`、batch=`0:0`（可另有 extern step）时，记录型 sacct 边界在无 `-X` 时返回 allocation+step 多行，有 `-X` 时只返回 allocation；生产 provider 必须恰查询一次并交出 `42:7`，随后真实 failure finalizer 先提交完整日志再删 exact work。删除 `-X` 的变异必须失败；即使带 `-X`，空、多非空行、多字段、命令失败/timeout 仍按原合同拒绝，不能静默取首行或去重。
...
- 独立失败退出码 provider 配记录型假 runner：terminal `JobRecord` → 精确 `sacct -j <job_id> -X -n -P --format=ExitCode` argv → 非空退出码字符串；不经 `poll`、不改 `JobRecord`
...
- 独立失败退出码 provider 收到 `FAILED`/`TIMEOUT` 的 terminal record（job ID `12345`），runner 返回 `"42:7\n"` -> runner 恰调用一次，argv 逐元素等于 `("sacct","-j","12345","-X","-n","-P","--format=ExitCode")`，返回值逐字为 `"42:7"`；成功/非终态 record 在 runner 零调用前被拒，空输出、多非空行、多字段与 runner 异常均抛 `ExecutorError` 且 `job_id == "12345"`
...
- **#47 已裁决为独立 `sacct ExitCode` provider**：轮询 `build_sacct_command` 仍不取 `ExitCode`；失败收尾 provider 对 terminal job 单独执行一次 `sacct -j <job_id> -X -n -P --format=ExitCode`，返回字符串作为 `FailureInputs.exit_code`。`JobRecord` 七字段不变，作业体自写退出码的备选不采用。真实站点输出形态仍须在 M4 oracle 复核，但生产 provider 的实现与 CLI 注入属于 M2 收尾任务。
...
- [x] 8.2 实现 work 内临时 registry/model manifest 生成与随 work 清理
- [x] 8.3 实现 SHUD 输入组装：变体 + forcing + `states/<source>/<T>.cfg.ic` → 运行目录；warm-start 状态 MUST 覆盖变体自带率定末态（可区分 IC fixture 断言）；固定覆盖六项参数（START=0/END=7/DT_QR_DOWN=60/Update_IC_STEP=720/BINARY_OUTPUT=1/ASCII_OUTPUT=0），00Z/12Z 同参数测试
...
1. 新增 yd 自撰公开模块 `producer/src/yd_producer/assemble.py` 与公开符号；允许一个同样 yd 自撰的私有 `producer/src/yd_producer/_assemble_fs.py` 承载 descriptor-bound copy/write/rename 组合以守住单文件 1000 行闸门，但不得导出第二条公共 seam、不得修改 shared `safe_fs`：
...
8. `<project_name>.tsd.forc` 只由 `contract.stations` 按 `shud_forcing_index` 生成：逐字 header `ID\tLon\tLat\tX\tY\tZ\tFilename`，每站一行 index/lon/lat/x/y/z/filename。lon/lat/x/y 与非负z必须用 `repr(float(value))` 等价的 shortest round-trip文本；既有`FileForcingRepository`把负elevation归一为0，故负z在该兼容index中逐字写`0.0`，读回也只按`max(contract.z, 0.0)`对拍。shared direct-grid parser 对已在 canonical `[-180,180)` 内的 longitude 必须原 float 返回（仅保留 `-0.0 -> 0.0`），不能用无条件模运算给合法 full-precision 值制造 ULP 漂移；只有 legacy `[180,360]` 输入继续减 360。direct-grid forcing station仍直接来自原contract，不得因兼容index改写z或其它geometry。该 index是 file repository 的兼容读面，按 index/filename/可观测geometry 与上述兼容规则对拍；`FileForcingRepository` 当前会按 `basin_slug` 派生自己的 met station id，故该 index不得反向定义/覆盖 contract `station_id`。
9. 整个 staged object-store模型树必须在本轮 work 内的唯一 nonce shadow object-store（`<work>/.<model_id>.registry-stage-<nonce>/object-store/models/<model_id>`）中以**最终相对 key**构造，并用真实 `FileForcingRepository` 读回 model identity、contract、validation assets和station index；全部通过后，把 staged model 子树跨目录但在同一 work/filesystem 中一次 no-follow rename 到 final `models/<model_id>`。这条取代不可执行的“同父 staging”措辞。final model root、caller 可预知的固定 staging、shadow staging或相关父层是不安全形态/预存条目时拒绝，不覆盖；nonce collision只拒绝该次调用。现有 shared rename 是裸 POSIX `renameat`、没有可移植 `RENAME_NOREPLACE`：受支持调用的单写保证来自#23 `run_with_lock`覆盖整个#26 controller生命周期，registry/run两个commit都须在rename紧前复探终名；测试必须证明“首次预检后、紧前复探前”出现的终名不被覆盖。不得伪称这能抵挡绕开共同runlock的外部并发写者。提交前失败清本次 staging，源 bytes不变；cleanup失败附到原异常。final rename已成功后的wrapper清理失败只进`cleanup_warnings`，不把成功伪报失败。
...
13. variant source必须绝对、已存在、逐分量 no-follow，且顶层恰有一个 `<project_name>.cfg.ic`普通文件与一个`<project_name>.para`普通文件；允许完整真实变体的其它普通文件/目录。树内任何symlink、FIFO/socket/device、不安全component或遍历IO错误均在final提交前拒绝。复制必须descriptor-bound/streaming，不把文件全量读入内存、不继承source mode/uid/gid；目录发现严格局限于显式variant根，既有path component grammar与宿主文件系统路径长度构成边界，**不得**无合同依据另设entry/depth业务上限。源树在组装前后全类型/bytes快照相等。
...
15. forcing authority只来自 `forcing.file_uris["package_manifest"]`：其key必须在同一WorkRegistry object-store与 `forcing.forcing_package_uri` 内；JSON经现有16 MiB bounded/no-follow loader等价纪律读，SHA-256逐字等于`forcing.checksum`；manifest的`source_id/cycle_time/model_id/forcing_version_id`与result/identity一致。#14 package manifest本身没有`forcing_package_uri`字段，故package identity由三方联合证明：result URI等于WorkIdentity唯一prefix、manifest key位于该prefix内、每个member URI逐字等于prefix + relative path；不得伪造或要求不存在的manifest字段。`status`只接受`forcing_ready|already_done`，checksum/file URI缺失或wrong-type拒绝。
16. manifest `files` 中必须恰有一个 `role="shud_forcing"`，relative path恰为canonical/legacy index之一；两者同时/零个拒绝。`shud_forcing_csv`项至少一条且不超过既有`MAX_DIRECT_GRID_STATION_BINDINGS=10000`，每项relative path为安全`shud/<basename>.csv`且basename casefold唯一；每个URI归一后必须等于package prefix + relative path。index与CSV均先在staging首写前descriptor-bound streaming预检，copy时再在同一输入fd核checksum以关闭swap窗口。index parser只保留当前字段状态与至多10000个已声明basename：逐chunk/逐字符识别header和恰七个tab字段，filename只接受现有ASCII component grammar；不得缓存整文件，也不得让无换行/超长字段使内存随输入字节线性增长。index解析出的filename multiset必须与CSV role basename精确相等；不扫描未声明文件、不接受debug `tsd_forc` fallback。运行目录中index改名`<project_name>.tsd.forc`，CSV basename原样落根；debug、package manifest、payload、domain/handoff/sidecar不落run dir。
17. 六项参数唯一writer为可单测纯函数`render_shud_parameters(content: bytes) -> bytes`（同模块公开，供#17补跑复用）：输入strict UTF-8且≤`MAX_OBJECT_MANIFEST_BYTES`。每个key只识别完整token `{{KEY}}`、`${KEY}`或行级`KEY = value`三类；零命中按输入主行尾风格在EOF追加，恰一命中只替换value/token，多命中（同形或跨形）拒绝，substring/注释里的普通文字不算。同一行可各含一个**不同** key 的placeholder，writer必须基于前一键已改写的结果继续替换，不能在后键处理时恢复旧placeholder。值逐字为`0/7/60/720/1/0`；六项之外所有bytes与行尾不变。函数不接cycle，因此同一模板的00Z/12Z bytes必须相等。
18. forcing/variant/state/registry全部验证完成后才发生assemble staging写。WorkRegistry point-of-use重验registry/model manifest仍受16 MiB JSON合同限制，但binding/`.sp.att`只受创建时显式`max_asset_bytes`：二次重验必须descriptor-bound streaming核contract checksum，**不得**把已合法提交的asset偷偷压回`MAX_OBJECT_MANIFEST_BYTES`。任何静态输入拒绝、copy/parameter写失败或rename前异常都保证final model不存在、四个source全树bytes/类型不变；清理每步互不取消，原始`AssemblyError`为主、cleanup failures进notes。final rename成功后不再回滚，wrapper cleanup失败进入`RunDirectory.cleanup_warnings`。#15不删整棵work；后继publish/cleanup负责。

**Must preserve / unchanged siblings**：

- #14 `ForcingProducer`、`FileForcingRepository`、package manifest/serializers、shared validator与53腿oracle保持不变；Round 1 C1 的 real-repository full-precision证据暴露 shared parser 对 canonical longitude 的无条件模运算会制造1 ULP漂移，故只允许在`direct_grid_contract.py::_required_longitude`做“canonical原值直返、legacy `[180,360]`减360”的窄修并补独立parser回归/变异，其余#14边界零修改；不能把其它assembler适配塞进snapshot。
...
- Shared helper roots: 只消费 `store/{safe_fs,object_store}.py`、`state`、`forcing` public symbols；除`direct_grid_contract.py::_required_longitude`按item 8窄修canonical float身份外零修改。
...
- registry happy path：显式GFS/IFS identity/contract/assets -> exact五个key/JSON keys/bytes；真实FileForcingRepository逐seam读回；binding/`.sp.att`/index bytes与独立literal相等。另用需超过10位有效数字的lon/lat/x/y/z与负z两臂证明shortest round-trip文本、canonical longitude 经shared parser float-exact round-trip和`z<0 -> 0.0`兼容视图；独立parser矩阵同时钉死`-0.0 -> 0.0`与`180/360` legacy归一，原contract geometry与ForcingProducer direct-grid station仍保持原值。
...
- registry commit故障：每个write/validation/rename/cleanup位置注入；rename前无final，清理继续；rename后cleanup warning不抛；在首次预检后、commit紧前复探前植入终名，registry与run两路均须零rename、既有bytes不变。证据明确只在`run_with_lock`单写模型内成立，不宣称POSIX rename-noreplace。
...
- forcing拒绝：status/checksum/result package URI/manifest-key prefix/manifest identity/member URI、oversize/deep/wide/malformed JSON、index canonical+legacy双份/零份、unsafe/duplicate/10001 CSV、URI外逃、checksum mismatch、index filename差集、debug fallback；尤其index filename在合法已声明basename后追加unsafe字符或超长suffix时，FSM必须消费自身invalid flag并在validate拒绝，不得截断回已声明prefix。全部零final、未声明外部对象不读。无换行超长index与跨chunk超长字段在tracemalloc/RSS结果中内存不随输入字节线性增长，且invalid UTF-8仍fail closed；不得只用源码grep证明streaming。
- variant/commit拒绝：source nested symlink/FIFO/device、preexisting final/staging、parameter duplicate、copy/write/rename/cleanup injected errors；`<project>.cfg.ic`与`<project>.para`各自missing/directory/symlink三态必须直接在validate拒绝，不得只由后继读失败间接覆盖；快照逐项证明四源不变与final可见性。另以`max_asset_bytes > 16 MiB`创建checksum-correct binding/`.sp.att`，手构/正常返回的WorkRegistry在assemble point-of-use继续流式校验并成功，不把asset读入内存或误套JSON manifest上限；checksum drift仍拒绝。
- 参数纯函数：三种shape、零命中append、CRLF/LF/no-final-newline、substring/comment、同一行不同key placeholders、duplicate同形/跨形、invalid UTF8/oversize；独立literal bytes。同一行不同key用例必须证明已替换值不会被后续key处理回滚。#17只import此函数不复制实现。
- mutation matrix（独立scratch、unset inherited venv、`uv run python -m pytest`、禁pyc、import provenance、控制变异、每腿独立restore）：至少杀死 WorkIdentity source/cycle guard、work exact path、binding checksum、sp checksum、contract URI、真实repository读回（含station row内容、geometry shortest-roundtrip/负z兼容与shared parser canonical-longitude原值直返）、pre-rename final可见与commit紧前复探、state path/header、warm-state覆盖退回variant、required variant cfg.ic/para regular preflight、forcing manifest checksum、result/manifest/member package identity三方、role-only选择、index filename set/流式有限状态/invalid-name flag、CSV数量上限与preflight/copy双checksum、point-of-use asset流式checksum、六参数每一值/duplicate guard、rename commit与cleanup evidence；报告实际红node与SHA restore，不预写红数。
...
- `producer/tests/test_assemble_run.py`：8.3 state/variant/commit边界；forcing package拒绝矩阵与参数纯函数分别拆到`test_assemble_forcing.py`/`test_assemble_parameters.py`，避免把一个测试文件堆到1000行。
...
- [x] 14.6 controller 将 #171 exact-five variant 与精确 cycle state 有界提交为 claimed-work capability，并新增不放宽 legacy guard 的 staged assemble seam（14.2 前置；issue #177，产品 PR #181 已合并）
...
- final guard 后 foreign exact root 赢得 claim -> 本源 raw RunError、零 staging/submit/DONE、foreign bytes+identity 保留、兄弟完整终止。
- controller claim 成功 -> `stage_raw` 只在既有 exact root 内建子树；删除 claim/退回 check-then-create 的 mutation 必红。
- GFS 创建 shared `work/` 后暂停，IFS raw copy 失败 rollback -> IFS 不删 shared/GFS 路径，GFS 继续 publish/catch-up；不得用全局 staging lock 让用例假绿。
- terminal 后把 `work` 父根重绑为 external same-layout tree -> external job.log 不被读、external tree不删、本源 cleanup RunError、兄弟完成。
- failure log commit 后 exact root 换 inode -> official log 保留、replacement 不删、cleanup RunError；跳过 delete 或按 pathname 删除两个方向各有判别器。
- publish scratch read 前 exact root 换 inode -> pre-DONE publish RunError、零 DONE、replacement 不删。
- DONE 已写后 exact root 换 inode -> `SUCCEEDED_CLEANUP_PENDING`、DONE/replacement 保留。
- standalone stage_raw/cleanup/publish 不传 claim -> 原 signatures/behaviors/nodeids 不变；direct run_once FAILED/TIMEOUT 仍保留 claimed work。
...
12. point-of-use race：early check 后、controller 的排他 exact-root claim 前让 competing writer 先创建 foreign marker/tree -> claim 的 `mkdir` 稳定失败为本源 `RunError(phase="raw")`，marker/tree bytes+identity 保留、零 staging/submit；兄弟完整追赶。退回 check-then-create、把 `FileExistsError` 当成功或绕过 claim 的 mutation 必红。
...
22. Round 1 ownership claim race：在 final `_reject_preexisting_work` 返回后、claim mkdir 前让 foreign exact root/marker 先创建 -> 本源 raw RunError、零 stage/submit/DONE，marker bytes+inode 不变，兄弟完整序列；另测 claim 成功后才进入 stage，移除 O_EXCL/exclusive mkdir 或退回 `lexists` 的 mutation 必红。
23. Round 1 shared ancestor rollback：事件暂停 GFS 于 shared `work/` 已创建而 source child 未创建的窄缝，让 IFS copy 失败 rollback 后恢复 GFS -> 仅 IFS error，GFS publish/DONE/后续缺口完整；把 shared ancestor 重新记入 `_Written.dirs` 的 mutation 必红，把 staging 加全局锁的变异由首轮 overlap/事件路径杀死。
24. Round 1 identity drift matrix：(a) terminal 后、FailureInputs 前把 `work` 父根换成指向 external same-layout tree的 symlink -> external log/tree 不读不删、cleanup error；(b) failure log commit 后把 exact root 换成新 inode -> official log保留、replacement保留、cleanup error；(c) publish scratch contract/read 前换 root -> 无 DONE 的 publish error；(d) DONE 后、work delete 前换 root -> cleanup-pending + DONE/replacement保留。删除 expected identity 比较、只入口比较不在最终 rmdir 前复验、事后 resolve 重绑、missing_ok 吞漂移的变异分别必红。
25. Round 1 later-failure evidence：每源至少一轮成功后，在后续轮参数化 FAILED/TIMEOUT、provider None/空白/抛错、failure log 写失败、work delete失败；error source partial tuple精确保留前轮，error cycle/job绑定失败轮，兄弟完整追赶。两源在不同轮次失败时两日志/exit/cycle/job/work逐项不串线。
...
- `rawcopy.py`：standalone API不扩；controller 已 claim exact root时只在其内建子树，`_Written` 不登记 shared ancestors。该文件既有 exclude 不扩大。
...
- `producer/tests/test_controller_sources_failures.py`：mapping/provider/cleanup/partial aggregate error 矩阵；接近上限时按 later-failure/ownership语义拆新文件，不加 exclude。
- `producer/tests/test_controller_crash_recovery.py`：unknown work、lstat、无 work 恢复、人工排除与 point-of-use claim race；接近上限时拆 `test_controller_work_claim.py`。
