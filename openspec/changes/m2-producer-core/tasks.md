...
8. `execute_retention_plan` 在第一处删除前对**全部仍存在目标**重复第 7 条 realpath/类型预检；这是抵御“计划后、执行前换成 symlink”的点-of-use 闸。二次预检之后，output 正式树用 `rmtree_no_follow`（树内任一 symlink 同样拒绝），失败日志用 `unlink_no_follow`，均带 `containment_root=plan.yd_root` 与 `missing_ok=True`。顺序为 output 后 logs；执行中途 IO 错误可部分完成，但旧 plan 重放与重新计划都必须幂等。
...
- `RetentionPlan` 身份绑定矩阵（Phase 2 审计 P0 的回归）：`source='ifs'` 夹带 GFS output 或 GFS log、夹带恰好 cutoff/窗口内路径、output/log lane 对调、cycle 非法、`cutoff != latest_done-14d`、空窗口带非空目标、未排序/重复目标、目标落在 resolved root 的兄弟路径 -> 构造或执行前 `CleanupError(phase='validate')`，所有目标逐项存活；反向正例：一份手工构造但字段全部自洽的 IFS 窗口外 plan 可执行且只删那两个精确目标
...
- 正式 output 目录内部放根外 symlink -> `rmtree_no_follow` 拒绝且根外目标/该树存活；unexpected regular-file/FIFO lane -> 稳定错误、不阻塞读取
