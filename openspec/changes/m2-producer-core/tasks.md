...
- **缺段的具体报错（spec Scenario「缺 river 段被拒 → 指明缺失段」）**：pin 返回三个 list，没有「段缺席」概念，`doc.river is None` 在 pin 那里表现为 `river row count 0 != expected N`。yd 侧 MUST 在段缺席时给出点名该段的 reason（如 `missing river section`），这是对 pin 的**扩展**（非偏离，pin 无对应面），由 spec Scenario 强制。段存在但行数不符仍走 `_check_row_counts` 的行数消息。
