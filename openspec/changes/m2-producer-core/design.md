...
**D19 prepared handoff（#202 native v2 取代 #171 的 five-only 格式）**：沿用每个 source 顶层 `yd.direct-grid-handoff.json`、既有四个模型版本字段、DirectGridForcingContract 与 opaque `yd.binding`，改为 `yd.prepare.direct-grid-handoff.v2` 与固定 `yd.sp.att`。完整变体是 native-yd-model-input D1/D2 的 14 文件，参数名为 `yd.cfg.para`；增加一个固定键的 file_checksums map，不增加角色/任意目标路径或另一套 registry。原有唯一 loader、限量 no-follow 读取、prepare 复制/四终名事务、#83 残留拒绝与唯一率定态语义继续使用，不新增逐层全包检查。旧 five-only 重新 prepare，不自动兼容。真实 prepare driver 业务代码改归 M2，M4 仅作现场验证；#132 自身六文件范围不承接这些前置实现。

**D20 claimed-work staged inputs（#202 扩展 native 文件集合）**：controller 仍在取得 exact work claim 后、提交作业前将 source 变体与 exact T state 搬入固定 `input/variant/`、`input/states/<source>/<T>.cfg.ic`。staged schema 升为 `yd.run.staged-inputs.v2`，沿用既有 canonical manifest/file-checksum map、source/cycle/work path 与 claim-relative IO，只将 five-only 集合替换为完整固定 native 集合。manifest 不是 job receipt，不新增模型 registry；设备号/inode 仍只作进程内 owner，不跨节点序列化比较。现有 timeout/crash 保留 work、成功/失败 exact-work 清理政策保持不变。

现有 `stage_work_inputs`、`load_staged_work_inputs`、`assemble_staged` 公共签名保持；capability 使用既有路径、root identity、manifest checksum、file-checksum map 与 prepared handoff，不另建防御框架。`assemble_staged` 经现有输入入口接受 native v2，复用共享 kernel 写到 `model/input/yd/`，返回真实嵌套 state/parameter/index 路径；forcing CSV 仍位于 model 根，index 的相对 path 行固定为 `.`，不改值/站点/Z。SHUD cwd、tracker/显式 -o 输出仍为 model 根。独立 legacy external-root `assemble` API、递归输入语义及其拒绝 work-relative raw paths 的合同不变；不增加 native v1 fallback。
...
**D21 #202 单流域 native 输入与真实 prepare driver**：以 [native-yd-model-input](../archive/2026-09-15-native-yd-model-input/design.md) 的固定文件/两 source 方案为当前生产合同，替代旧 prepare-unavailable/M4-driver 归属。prepare-only driver 使用指定 NWM 解释器直接调用库，不引入 NWM 平台审批、snapshot UUID、DB、通用资产角色或多层整包验证；日常 run 仍只使用 yd。旧 issue fixture 的 five-only/flat/未实现分支约束仅解释对应历史提交，不要求新实现继续保留错误布局；原 state、cleanup、publish、WorkClaim 与 #132 review ledger 不变。格式迁移与真实 builder 两个前置切片完成后才继续 #132 合并。
