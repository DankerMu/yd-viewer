...
controller 创建的 `PublishInputs` 与 `FailureInputs` 必须携带该 claim；controller 调用 `rawcopy.stage_raw` 时也必须传 claim，使目标目录创建/文件 O_EXCL/rollback 都相对一个已打开且 identity 匹配的 exact-root fd 执行，不能在每步重新走 pathname。为不破坏 standalone userspace，这三个入口只允许在参数末尾增加默认 `None` 的 additive claim；既有调用不传时行为不改，controller 路径则必须非空并在读取 scratch、提交正式产物和删除前重验 no-follow identity。`safe_fs.remove_tree_allow_symlinks` 同样只允许增加默认 `None` 的 expected-root identity；传入时先比较 named entry 与 opened fd，并在最终 `rmdir` 前再次比较，identity 漂移时以 `kind="identity_changed"` 拒绝且零 replacement 删除。failure 日志已提交后发现漂移，保留日志与 replacement，收敛为 `RunError(phase="cleanup")`；`DONE` 已写后发现漂移，保留 replacement 并返回 `SUCCEEDED_CLEANUP_PENDING`；`DONE` 前漂移为 raw/collect/publish `RunError`，不得落 `DONE`。直接六参数 `run_once` 仍使用 claim，但 FAILED/TIMEOUT 不自动 finalizer、work 仍保留。
