...
production driver 的 `prepare(request)` 只能以 #134 的显式来源构造 worker 输入：`source_id`/`cycle_time` 逐字取 `request.source`/`.cycle`，`project_name` 只从 `prepare.calibrated_state_path(request.variant_dir)` 指向、prepare 已验证的率定状态文件名仅移除末尾一次 `.cfg.ic` 后取得；不扩 config/local schema，也不从 variant basename、binding、环境或数据库反造。四个 versioned registry identifier 由该 driver 为单一 attempt 唯一拥有，只在本 work 的 registry/forcing/assemble/receipt 中一致使用，M2 不编造生产值或声称外部 authority。`DirectGridForcingContract` 只取 prepare-owned 验证 handoff，不从 raw builder 文件在运行时反推；`yd.binding` 只取 prepare 已验证的 `request.variant_dir/yd.binding` 精确 bytes，`.sp.att` 只取 handoff 指名的精确 bytes；driver/worker 都只对 handoff 明示的 asset path 作 bounded no-follow 读取，并核验 handoff source/cycle/project/work identity、contract current-source identity 和声明的 SHA-256。worker receipt 必须携带同一 `WorkIdentity`（含四个 identifier）及两份 asset checksum。`contract.binding_uri`/`contract.sp_att_path` 是 D11 的提交后 work-local keys，不是读取路径；hand-off contract 不满足该形状时同样在 submit 前 fail closed。当前没有获证 `.sp.att` 实体位置/parser，不能把这些 key 当作变体发现规则。独立进程的 M2 合成 handoff 必须显式携带 contract、两份 bytes、预定 SHA-256 和同一 attempt identity，且逐项篡改均拒绝；这只证明本地进程交接，不是现场 builder 值。
...
#134 把 production driver 的来源逐字段钉死：`source_id` 与 `cycle_time` 只逐字取 controller 交给 `prepare(request)` 的 `AttemptRequest.source` / `.cycle`；`project_name` 只从 `prepare.calibrated_state_path(request.variant_dir)` 指向、prepare 已验证的率定状态文件名仅移除末尾一次 `.cfg.ic` 后取得，绝不取变体目录 basename。`model_id`、`basin_id`、`basin_version_id`、`river_network_version_id` 是该 driver 为**一次 attempt**唯一持有的一组 versioned、work-local registry identifier：M2 不给它们持久或外部权威、不内置生产字符串，四值只贯穿同一 work 的 registry → forcing → assemble 并进入同一 worker receipt；M4 才拿 node-22 真实 builder/site artifact 对账。任何字段都不得从 TOML、环境、`DATABASE_URL`、NWM PostgreSQL/服务型 registry、目录扫描、`yd.binding` 或测试 fixture 推导。

调用方仍显式提供 identity、#14 的 `DirectGridForcingContract` 以及同一已验证 binding/`.sp.att` bytes；production driver 只可消费 prepare-owned 验证 handoff：contract 不从 raw builder 文件在运行时反推，binding 只读 prepare 已验证的 `variant_dir/yd.binding`，`.sp.att` 只读 handoff 明示路径。driver/worker 以显式 `max_asset_bytes` 为界作有界 no-follow 读取，并核验 handoff source/cycle/project/work identity、contract current-source identity 和声明的 SHA-256 后交给 worker。当前 M2 没有获证的真实 `.sp.att` 位置/parser，`contract.sp_att_path` 仅是 registry 提交后的相对 key，不能反推其来源；缺少 prepare 明示交接即在 submit 前 fail closed。M2 只用独立进程的合成 handoff（显式 contract、`yd.binding`/UTF-8 `.sp.att` literal bytes 及独立预先固定的 SHA-256）证明该链和篡改拒绝；M4 才验证真实 builder 布局、Slurm/NFS/SHUD 与现场 identifier 值，不能把合成 fixture 升格为生产事实。
