...
**D3 Slurm 注入式执行器**：控制器依赖 `JobExecutor` 协议（submit/poll 语义），生产实现封装 `sbatch`/`sacct`，测试注入进程内 fake（同步完成/失败/超时可编排）。备选是 subprocess mock，弃——mock 命令行参数脆弱且测不到状态机。真实提交行为属 M4 oracle。
...
**D5 依赖策略**：骨架 `dependencies = []` 为刻意留空；numpy/xarray/cfgrib 等随 forcing-chain 对应任务加入并 `uv lock`。CLI 用 stdlib `argparse`，零框架依赖（KISS；三个子命令不值 click/typer）。
...
4. `forcing.build(work, manifest) -> ForcingPackage`（合成 canonical fixture）——§13.1 "DB-free 链"行的验收边界；快照模块另带 NWM 来源最小测试。
...
- [cfgrib/eccodes 在 CI ubuntu 上安装失败] → 优先二进制 wheel；不行则 CI producer job 补 `apt-get install libeccodes0`，在 forcing-chain 任务内解决并验证 CI 绿。
