...
production driver 的 `prepare(request)` 只能以 #134/#171 的显式来源构造 worker 输入：`source_id`/`cycle_time` 逐字取 `request.source`/`.cycle`，`project_name` 只从 `prepare.calibrated_state_path(request.variant_dir)` 指向、prepare 已验证的率定状态文件名仅移除末尾一次 `.cfg.ic` 后取得；不扩 config/local schema，也不从 variant basename、binding、环境或数据库反造。driver 以 exact `request.variant_dir`、current source/project/grid 与显式 limits 调 #171 唯一 loader，逐字取得四个 prepared-variant 版本标识、深冻结 `DirectGridForcingContract` 与 exact binding/`.sp.att` bytes；不得逐 attempt 随机生成标识、扫描 asset 或复制 parser。driver/worker 将其与本次 source/cycle/work identity 绑定并在 claimed work 使用前重验，worker receipt 必须携带同一 `WorkIdentity` 及两份 asset checksum。`contract.binding_uri`/`contract.sp_att_path` 是 D11 的提交后 work-local keys，不是 prepared asset 读取路径；handoff 不满足 schema/identity/checksum/path 形状时在 submit 前 fail closed。独立进程 M2 合成链必须从 #171 loader 开始并逐项篡改拒绝；这只证明本地 carrier/进程交接，不是现场 builder 值。

生产 driver/worker 仍必须守住本段的安全理由：重计算只在 Slurm job 内执行，worker 以原子、checksum/identity 绑定的 work-local receipt 传递同一 tracker authority；不得用目录扫描、提交前预埋产物或测试 terminal hook 冒充生产接线。M4 只做 node-22 真 Slurm/NFS/SHUD receipt、真实 builder/site layout 与 identifier 对账、现场值与 cron 安装，不再负责补写 CLI 业务体。
...
#134 把 production driver 的来源逐字段钉死，#171 补足跨独立 `prepare`/`run` 进程的持久 carrier：`source_id` 与 `cycle_time` 只逐字取 controller 交给 `prepare(request)` 的 `AttemptRequest.source` / `.cycle`；`project_name` 只从 `prepare.calibrated_state_path(request.variant_dir)` 指向、prepare 已验证的率定状态文件名仅移除末尾一次 `.cfg.ic` 后取得，绝不取变体目录 basename。`model_id`、`basin_id`、`basin_version_id`、`river_network_version_id` 是 builder 声明并随同一 prepared variant 稳定提交的版本化属性；production driver 只能从 #171 唯一 loader 的深冻结返回值逐字取得并复制到本 attempt 的 `WorkIdentity`，不得逐 attempt 随机生成或内置生产字符串。四值只贯穿同一 work 的 registry → forcing → assemble → receipt；M4 以 node-22 真实 builder/site artifact 对账。任何字段都不得从 TOML、环境、`DATABASE_URL`、NWM PostgreSQL/服务型 registry、目录扫描、variant basename、`yd.binding` 内容或测试 fixture 推导。

调用方仍显式提供 identity、#14 的 `DirectGridForcingContract` 以及同一已验证 binding/`.sp.att` bytes；production driver 只可消费 #171 的 prepared-variant handoff loader。loader 固定读取 `yd.direct-grid-handoff.json` 与其明示的单层 `.sp.att` asset，binding 只读同变体 `yd.binding`；它以显式 manifest/asset limits 作 bounded no-follow 读取，并核验 strict schema、source/project/grid、四个版本标识、contract current-source/D11 key shape 与两资产 SHA-256。driver 在 claimed work 内再加 source/cycle/work 形成 attempt handoff，worker 使用前重验。`contract.sp_att_path` 仍仅是 registry 提交后的相对 key，不能反推 prepared asset 路径；缺失/非法 handoff 在 submit 前 fail closed。M2 只用独立进程 synthetic builder/loader/worker 链证明 carrier 与篡改拒绝；M4 才验证真实 builder layout/parser、Slurm/NFS/SHUD 与现场 identifier 值，不能把合成 fixture 升格为生产事实。
