...
claim 本身不改变 rawcopy 的既有“写入失败不留半套”与 compute-loop 的“下次从干净 work 重试”合同：`stage_raw` 已成功回滚本轮所有后代后，controller 必须只在 exact root 仍匹配 claim 且为空时以 identity-bound `rmdir` 释放它；不得递归删除、不得删除 source/shared ancestor。root 非空、identity 漂移或清理无法确定时保留当前 entry，并把清理失败证据附加到原 `RunError(phase="raw")`，不能把本进程创建的空根留成下一 tick 的 `UNVERIFIED_WORK_RESIDUE`。`_work_claim` 的每个 fd walk 在 `Exception` 与 `BaseException` 路径都必须关闭本调用打开的 root/descendant fd，且不能关闭成功返回给 caller 的文件 fd；资源生命周期以可计数 open/close 的注入测试与变异体闭合。
