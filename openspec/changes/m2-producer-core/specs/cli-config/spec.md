...
生产 driver MUST 让 canonical/forcing/assemble/SHUD/tracker/recovery 在 Slurm job 内执行，并通过原子、checksum/identity 绑定的 work-local receipt 把同一 source/cycle/work/job 的 `RunDirectory`、DAT、merged log 与已验证 T+12 checkpoint 交给 `collect`；receipt 还 MUST 绑定同一 `WorkIdentity`（含四个 work-local identifier）与 direct-grid binding/`.sp.att` checksum。登录节点不得补跑或从规范文件名重建 checkpoint authority。M4 只负责 node-22 真实 Slurm/NFS/SHUD receipt 与 cron 安装，不负责补写 CLI 业务体。

production driver 的 `WorkIdentity` 与 direct-grid 输入 MUST 走唯一的显式 handoff：`source_id`、`cycle_time` 逐字来自 controller 的 `AttemptRequest.source`、`.cycle`；`project_name` 只从 `prepare.calibrated_state_path(request.variant_dir)` 指向、prepare 已验证的率定状态文件名仅移除末尾一次 `.cfg.ic` 后取得，MUST NOT 从 `variant_dir` basename 推导。既有 `config.toml` / `local.toml` identity schema 保持闭合，不新增字段。`model_id`、`basin_id`、`basin_version_id`、`river_network_version_id` 是 production driver 为该 attempt 唯一持有的一组 versioned、work-local registry identifier：只在同一 scratch 的 registry → forcing → assemble 链内使用，并逐字记录进 worker receipt；M2 不赋予它们跨轮持久或外部权威，M4 以真实 node-22 builder/site artifact 对账。

worker 只可消费由 prepare 已验证变体显式交接的 `DirectGridForcingContract`、精确 `yd.binding` bytes 与 `.sp.att` bytes：handoff 是一次 attempt 的显式 contract、`.sp.att` path、两份 checksum 与 source/cycle/project/work identity 参数集合，不是新增 config/local schema 或持久文件格式；`yd.binding` 只读 prepare 已验证的 `request.variant_dir/yd.binding`，contract 与 `.sp.att` 只读该 prepare-owned 验证 handoff；M2 不从 raw builder 文件为后两者发明生产路径/parser。driver 在登录节点把这些资产放入已认领 work 前、worker 在使用 handoff 前，都 MUST 作有界、逐分量 no-follow 普通文件读取，并核验 handoff 的 source/cycle/project/work identity、contract 的 current-source identity 及其声明的 SHA-256；不得从 TOML、环境、`DATABASE_URL`、NWM PostgreSQL/服务型 registry、目录扫描、变体 basename、`yd.binding` 内容或测试 fixture 推导 identity、contract、路径或 bytes。`contract.binding_uri`/`contract.sp_att_path` 只是提交后 registry 的 relative keys，不得作为变体发现路径；M2 当前没有获证的真实 `.sp.att` 物理位置/parser，故 handoff 不能提供 D11 work-local key shape 的 contract 与明示 asset path 时 driver MUST 在 submit 前拒绝，MUST NOT 猜变体子路径。M2 只以独立进程的合成 handoff 验证此调用链：fixture 显式提供同源 contract、`yd.binding`、UTF-8 `.sp.att`、各自预先固定 SHA-256、率定状态文件名与一次 attempt 的 opaque identifier；任一 bytes/checksum/source/cycle/project/work 绑定被篡改时零 receipt/DONE。M4 才验证真实 builder 布局、stage mapping、site 值、Slurm/NFS/SHUD，不得把该合成 oracle 当作生产事实。
...
- **WHEN** driver 收到缺少 prepare handoff 的 `.sp.att`、将 `contract.sp_att_path` 当作变体发现路径、从变体 basename/TOML/环境/数据库/目录扫描/`yd.binding` 内容/测试 fixture 推导任一 identity 或 asset，任一 handoff leaf/ancestor 是 symlink 或非普通文件、超过显式 `max_asset_bytes`，或任一 handoff bytes、SHA-256、source/cycle/project/work identity、contract current-source identity 不一致
- **THEN** 在 `sbatch` 前 fail closed，零 worker receipt、零 DAT、零 `DONE`；M2 的独立进程合成 fixture 只以 `AttemptRequest.source`/`.cycle`、`<synthetic-project>.cfg.ic` 去后缀的 project、singleton-source contract、`variant_dir/yd.binding`/显式 UTF-8 `.sp.att` literal bytes、固定 checksum 和 opaque work-local identifier 覆盖此边界，真实 builder `.sp.att` layout/parser 与 site 值仍由 M4 核验
