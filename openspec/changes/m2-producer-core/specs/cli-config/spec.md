...
production driver 的 `WorkIdentity` 与 direct-grid 输入 MUST 走唯一的 prepared-variant handoff：`source_id`、`cycle_time` 逐字来自 controller 的 `AttemptRequest.source`、`.cycle`；`project_name` 只从 `prepare.calibrated_state_path(request.variant_dir)` 指向、prepare 已验证的率定状态文件名仅移除末尾一次 `.cfg.ic` 后取得，MUST NOT 从 `variant_dir` basename 推导。既有 `config.toml` / `local.toml` identity schema 保持闭合，不新增字段。`model_id`、`basin_id`、`basin_version_id`、`river_network_version_id` 必须逐字来自 `load_prepared_variant_handoff` 返回的同一 prepared-variant 稳定版本标识；driver 不得逐 attempt 随机生成或从资产内容补值。它们只在同一 scratch registry → forcing → assemble → receipt 链内使用；M4 以真实 node-22 builder/site artifact 对账，M2 synthetic 值不构成现场权威。

worker 只可消费由 prepare 同一 loader 验证并随变体提交的 `DirectGridForcingContract`、exact opaque binding bytes 与 manifest 明示的 UTF-8 `.sp.att` bytes。登录侧 driver 以 `request.variant_dir`、current source/project/grid 与显式 limits 调唯一 loader，不读 config identity、环境、`DATABASE_URL`、NWM PostgreSQL/服务型 registry，不扫描 manifest/asset，也不从变体 basename、`yd.binding` 内容或测试 fixture 推导 identity、contract、路径或 bytes。`contract.binding_uri`/`contract.sp_att_path` 只是提交后 registry 的 relative keys，不得作为 prepared asset 读取路径。driver 把已验证模型级 snapshot 加上 source/cycle/work 写入 claimed-work attempt handoff；worker 使用前重验，receipt 再绑定同一 source/cycle/work/job/`WorkIdentity` 与两资产 checksum。缺 manifest/明示 asset、schema/identity/checksum/path 不一致或文件形态不可安全确认时 MUST 在 `sbatch` 前拒绝。M2 独立进程 fixture 只证明合成 builder→loader→worker→receipt 调用链；M4 才验证真实 builder `.sp.att` layout/parser、stage mapping、site 值、Slurm/NFS/SHUD，不得把合成 oracle 当生产事实。
...
- **THEN** 在 `sbatch` 前 fail closed，零 worker receipt、零 DAT、零 `DONE`；M2 的独立进程合成 fixture 只用已由 #171 唯一 loader 接受的 prepared variant，并另以 `AttemptRequest.source`/`.cycle/work` 构造 attempt identity，真实 builder `.sp.att` layout/parser 与 site 值仍由 M4 核验
