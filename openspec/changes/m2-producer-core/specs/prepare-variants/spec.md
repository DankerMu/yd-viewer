...
`prepare` MUST 在任何检查与写入之前校验 `local.toml` 的 `yd_root` 与 `scratch_root` 均为**绝对路径**且已存在为目录；不满足即拒绝执行。清理与回滚 MUST 逐步独立执行：任一步失败 MUST NOT 取消其余步骤，MUST NOT 替换或掩盖在途异常，也 MUST NOT 把已完成提交的运行报成失败。
