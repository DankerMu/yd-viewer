...
#### Scenario: 命中刷新陈旧 mtime
- **WHEN** 缓存中已有 PNG 瓦片且其 mtime 为 25 h 前，请求该瓦片
- **THEN** 响应 `X-Tile-Cache: hit`，文件 mtime 被刷新到请求时刻附近，上游未被调用

#### Scenario: 新鲜瓦片不刷新
- **WHEN** 缓存中已有 PNG 瓦片且其 mtime 为 1 h 前，请求该瓦片
- **THEN** 响应 `X-Tile-Cache: hit`，文件 mtime 不变

#### Scenario: 刷新失败不影响命中
- **WHEN** 缓存中已有 mtime 为 25 h 前的 PNG 瓦片，且 `os.utime` 抛 `PermissionError`
- **THEN** 响应 200、`X-Tile-Cache: hit`、正文为该瓦片

#### Scenario: 读后文件被并发删除
- **WHEN** 缓存中 mtime 为 25 h 前的 PNG 瓦片在读出正文之后、`os.utime` 之前被删除（`os.utime` 抛 `FileNotFoundError`）
- **THEN** 响应 200、`X-Tile-Cache: hit`、正文为读出的瓦片
